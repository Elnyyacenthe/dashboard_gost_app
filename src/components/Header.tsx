...
export default function Header({ profile, onSignOut, isSuperAdmin = false, onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between gap-3 border-b border-border bg-white/80 px-4 backdrop-blur-xl sm:px-6">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="truncate text-[10px] font-bold uppercase tracking-[0.2em] text-primary-dark">
            Plugbet · Console
          </p>
          <h2 className="truncate hero-number text-base text-text sm:text-lg">
            Bonjour, <span className="text-primary-dark">{profile?.username ?? 'Admin'}</span>
          </h2>
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-1 sm:gap-3">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -right-0.5 -top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-extrabold text-white">
        ...
        <div ref={menuRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-border bg-white p-1.5 pr-2.5 transition-colors hover:border-primary-dark/40 hover:bg-surface-lighter"
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[#7CCD3F] to-[#5FAF2D] text-sm font-extrabold text-white">
